...
new Vue({
  ...
  template: `
    <div class="import-view">
      ...
      <a-button
        class="operation-button"
        @click="touchIYLabPage"
      >
        Ice Year 实验室
        <a-icon type="experiment" />
      </a-button>      
      <!-- 提示 -->
      <span class="import-hint">小提示：上传名单只支持 .xlsx、.xls、.csv 文件格式，纯名单即可！该系统内建缓存机制，在不重置数据或更换浏览器的情况下只需上传一次名单便可长期使用。启蛰六班的名单请下载启蛰模板后上传。</span>
      <!-- 自定义抽奖组件 -->
  ...
  methods: {
    ...
    // 下载模板
    touchQiZhePage () {
      window.location.href = './qizhe.csv'
    },
    // 跳转 v1
    touchV1Page () {
    ...
    // 跳转Ice Year实验室
    touchIYLabPage () {
      window.location.href = 'https://blog.iceyear.ml/lab'
    },    
    // 抽奖模式切换
